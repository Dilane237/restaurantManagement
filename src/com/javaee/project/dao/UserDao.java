...
import java.sql.*;
import java.util.*;
import com.javaee.project.model.User;
...
public class UserDao {
    ...
    public void addUser(User user) {
        try {
            PreparedStatement preparedStatement = connection.prepareStatement("insert into users(uname, password, email, registeredon, image) values (?, ?, ?, ?, ? )");
            // Parameters start with 1
            preparedStatement.setString(1, user.getUname());
            preparedStatement.setString(2, user.getPassword());
            preparedStatement.setString(3, user.getEmail());
            preparedStatement.setDate(4, new java.sql.Date(user.getRegisteredon().getTime()));
            preparedStatement.setString(5,user.getBase64Image());
    ...
    public List<User> getAllUsers() {
        ...
        try {
            ...
            while (rs.next()) {
                ...
                user.setBase64Image(rs.getString("image"));
                users.add(user);
            }
        } catch (SQLException e) {
            e.printStackTrace();
